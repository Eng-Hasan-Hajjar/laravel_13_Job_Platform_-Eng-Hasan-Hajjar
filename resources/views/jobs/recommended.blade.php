...
@section('content')
    <div class="page-container">
        ...
        <!-- Header Banner -->
        <div class="card"
            style="margin-bottom:2rem;background:linear-gradient(135deg,#1e3a5f,#2563eb,#7c3aed);border:none;overflow:visible">
            <div class="card-body" style="padding:2rem;color:white;position:relative">
                <div style="position:absolute;inset-inline-end:1.5rem;top:50%;transform:translateY(-50%);font-size:5rem;opacity:.2">🤖</div>
                <div style="position:relative;z-index:1;max-width:600px">
                    <div
                        style="display:inline-flex;align-items:center;gap:.5rem;background:rgba(255,255,255,.15);border-radius:var(--radius-full);padding:.375rem .875rem;font-size:.8rem;font-weight:600;margin-bottom:1rem">
                        <i class="fas fa-magic"></i> {{ __('messages.powered_by_ai') }}
                    </div>
                    <h1 style="font-size:1.75rem;font-weight:800;margin-bottom:.5rem">{{ __('messages.recommended_jobs') }}
                    </h1>
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:.5rem">
                        @foreach(auth()->user()->skills ?? [] as $skill)
                            @if($loop->index < 6)
                                <span
                                    style="background:rgba(255,255,255,.2);padding:.25rem .75rem;border-radius:var(--radius-full);font-size:.8rem;font-weight:600">
                                    {{ $skill }}
                                </span>
                            @endif
                        @endforeach
                        @if(empty(auth()->user()->skills))
                            <a href="{{ route('user.profile') }}"
                                style="background:rgba(255,255,255,.2);padding:.375rem .875rem;border-radius:var(--radius-full);font-size:.8rem;font-weight:600;text-decoration:none;color:white">
        ...
        <div style="display:flex;align-items:center;gap:1.25rem;margin-bottom:1.5rem;flex-wrap:wrap">
            <span
                style="font-size:.875rem;font-weight:700;color:var(--text-secondary)">{{ __('messages.match_score') }}:</span>
...
@endif
                        ...
                        @php
                            ...
                            if (count($matched) > 0)
                                $reasons[] = count($matched) . ' ' . __('messages.matching_skills');
                            if (in_array($job->type, auth()->user()->preferred_job_types ?? []))
                                $reasons[] = __('messages.preferred_type');
                            if ($job->experience_level === auth()->user()->experience_level)
                                $reasons[] = __('messages.matches_experience');
                        @endphp
                        @if(!empty($reasons))
                            <div
                                style="font-size:.75rem;color:var(--text-muted);margin-bottom:.875rem;display:flex;align-items:center;gap:.375rem;flex-wrap:wrap">
                                <i class="fas fa-robot" style="color:var(--primary)"></i>
                                @foreach($reasons as $r)<span
                                style="background:var(--bg-hover);padding:.15rem .5rem;border-radius:var(--radius-full)">{{ $r }}</span>@endforeach
                            </div>
                        @endif

                        <div class="job-card-footer">
                            <span class="job-posted-date"><i class="fas fa-clock"></i>
        ...
        @if($jobs->isNotEmpty())
            <div class="card" style="margin-top:2rem;text-align:center;padding:1.5rem">
                <i class="fas fa-sliders-h"
                    style="font-size:1.5rem;color:var(--primary);margin-bottom:.625rem;display:block"></i>
                <h3 style="font-size:1rem;font-weight:700;margin-bottom:.375rem">{{ __('messages.improve_recommendations') }}
                </h3>
                <p style="font-size:.875rem;color:var(--text-secondary);margin-bottom:1rem">
                    {{ __('messages.improve_recommendations_desc') }}</p>
