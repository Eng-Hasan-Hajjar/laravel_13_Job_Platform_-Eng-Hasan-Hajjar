...
@section('content')
                    ...
                    <div class="card-body">
                        <div style="display:flex;align-items:flex-start;gap:1.25rem;flex-wrap:wrap">
                            ...
                            <div style="flex:1;min-width:0">
                                <div
                                    style="display:flex;align-items:flex-start;justify-content:space-between;gap:.75rem;flex-wrap:wrap">
                                    <div>
                                        <h1 style="font-size:1.5rem;font-weight:800;margin-bottom:.375rem">{{ $job->title }}
                                        </h1>
                                ...
                                <div class="job-meta" style="margin-top:.75rem">
                                    @if($job->location)
                                        <span class="job-tag location"><i class="fas fa-map-marker-alt"></i>
                                            {{ $job->location }}</span>
                                    @endif
                                    @if($job->type)
                                        <span class="job-tag type"><i class="fas fa-clock"></i>
                                            {{ __('messages.' . str_replace('-', '_', $job->type)) }}</span>
                                    @endif
                                    @if($job->is_remote)
                                        <span class="job-tag remote"><i class="fas fa-wifi"></i>
                                            {{ __('messages.remote') }}</span>
                                    @endif
                                    @if($job->experience_level)
                                        <span class="job-tag"
                                            style="background:var(--bg-hover);color:var(--text-secondary);border-color:var(--border)">
                                            <i class="fas fa-layer-group"></i>
                                            {{ __('messages.exp_' . $job->experience_level) }}
                                        </span>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            <div style="display:flex;flex-wrap:wrap;gap:.5rem">
                                @foreach($job->skills as $skill)
                                    <span
                                        style="padding:.375rem .875rem;background:var(--primary-light);color:var(--primary);border-radius:var(--radius-full);font-size:.8rem;font-weight:600;border:1px solid rgba(37,99,235,.2)">
                                        {{ $skill }}
                                    </span>
                                @endforeach
                            </div>
            ...
            <div style="position:sticky;top:calc(var(--navbar-height) + 1rem)">
                ...
                <div class="card" style="margin-bottom:1.25rem;border-top:3px solid var(--primary)">
                    <div class="card-body">
                        @if($job->salary_min || $job->salary_max)
                            <div style="text-align:center;margin-bottom:1.25rem">
                                <div style="font-size:1.5rem;font-weight:800;color:var(--primary)">
                                    ...
                                    @endif
                                    <span
                                        style="font-size:.875rem;font-weight:500;color:var(--text-muted)">{{ $job->salary_currency ?? 'USD' }}/{{ __('messages.month') }}</span>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($job->deadline)
                            <div
                                style="display:flex;align-items:center;gap:.5rem;font-size:.8rem;color:var(--text-secondary);margin-bottom:1rem;padding:.625rem;background:var(--bg-hover);border-radius:var(--radius)">
                                ...
                                @elseif($job->deadline->diffInDays() <= 3)
                                    <span class="status-badge pending"
                                        style="font-size:.7rem">{{ __('messages.ending_soon') }}</span>
                                @endif
                            </div>
                        @endif
                        ...
                        <div style="display:flex;flex-direction:column;gap:.75rem;font-size:.85rem;margin-bottom:1.25rem">
                            <div
                                style="display:flex;justify-content:space-between;padding-bottom:.5rem;border-bottom:1px solid var(--border)">
                            ...
                            <div style="display:flex;justify-content:space-between">
                                ...
                                <span
                                    style="font-weight:600">{{ __('messages.' . str_replace('-', '_', $job->type)) }}</span>
                            </div>
                        </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div
                            style="font-size:.85rem;color:var(--text-secondary);line-height:1.6;margin-bottom:1rem;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden">
                        ...
                        <div style="display:flex;flex-direction:column;gap:.5rem;font-size:.8rem;margin-bottom:1rem">
                            ...
                            @if($job->company->location)
                                <span style="color:var(--text-muted)">
                                    <i class="fas fa-map-marker-alt" style="width:16px;color:var(--primary)"></i>
                                    {{ $job->company->location }}
                                </span>
                            @endif
                            @if($job->company->employees_count)
                                <span style="color:var(--text-muted)">
                                    <i class="fas fa-users" style="width:16px;color:var(--primary)"></i>
                                    {{ $job->company->employees_count }} {{ __('messages.employees') }}
                                </span>
                            @endif
                        </div>
                        ...
                        @if($job->company->reviews_count > 0)
                            <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:1rem">
                                ...
                                <span
                                    style="font-weight:700;font-size:.9rem">{{ number_format($job->company->average_rating, 1) }}</span>
                                <span style="font-size:.8rem;color:var(--text-muted)">({{ $job->company->reviews_count }}
                                    {{ __('messages.reviews') }})</span>
                            </div>
                        @endif

                        <a href="{{ route('companies.show', $job->company) }}" class="btn btn-outline"
    ...
    @auth
        @if(auth()->user()->role === 'user' && !$hasApplied)
            <div id="applyModal"
                style="display:none;position:fixed;inset:0;z-index:9999;align-items:center;justify-content:center;background:rgba(0,0,0,.5);backdrop-filter:blur(4px)">
                        ...
                        <form action="{{ route('jobs.apply', $job) }}" method="POST" enctype="multipart/form-data" data-validate>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="cover_letter" class="form-control" rows="5"
                                    placeholder="{{ __('messages.cover_letter_placeholder') }}" required></textarea>
                            </div>
                            ...
                            @if(auth()->user()->cv_path)
                                <div class="form-group">
                                    ...
                                    <div style="display:flex;flex-direction:column;gap:.75rem">
                                        <label
                                            style="display:flex;align-items:center;gap:.75rem;padding:.875rem;border:1px solid var(--border);border-radius:var(--radius);cursor:pointer">
                                            <input type="radio" name="cv_option" value="existing" checked>
                                            <div>
                                                <div style="font-weight:600;font-size:.875rem">{{ __('messages.use_existing_cv') }}
                                                </div>
                                                <div style="font-size:.75rem;color:var(--text-muted)">
                                                    {{ basename(auth()->user()->cv_path) }}</div>
                                            </div>
                                        </label>
                                        <label
                                            style="display:flex;align-items:center;gap:.75rem;padding:.875rem;border:1px solid var(--border);border-radius:var(--radius);cursor:pointer">
                            ...
                            <div class="form-group" id="cvUploadGroup">
                                ...
                                <div class="upload-zone">
                                    ...
                                    <i class="fas fa-cloud-upload-alt"
                                        style="font-size:2rem;color:var(--text-muted);margin-bottom:.5rem"></i>
                                    <div style="font-weight:600;margin-bottom:.25rem">{{ __('messages.drop_cv_here') }}</div>
                                    <div style="font-size:.8rem;color:var(--text-muted)">PDF, DOC, DOCX •
                                        {{ __('messages.max_size') }}: 5MB</div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="number" name="expected_salary" class="form-control" placeholder="0" min="0">
