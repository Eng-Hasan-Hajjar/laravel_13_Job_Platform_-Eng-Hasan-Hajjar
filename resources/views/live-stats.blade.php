@extends('layouts.app')
@section('title', __('messages.live_statistics'))
...
<div class="page-container">
    ...
    <div class="grid grid-4" style="margin-bottom:2rem">
        <div class="stat-card primary"><div class="stat-card-icon"><i class="fas fa-briefcase"></i></div>
            <div><div class="stat-value" id="stat-jobs">0</div><div class="stat-label">{{ __('messages.active_jobs') }}</div></div>
        </div>
        <div class="stat-card success"><div class="stat-card-icon"><i class="fas fa-users"></i></div>
            <div><div class="stat-value" id="stat-users">0</div><div class="stat-label">{{ __('messages.job_seekers') }}</div></div>
        </div>
        <div class="stat-card warning"><div class="stat-card-icon"><i class="fas fa-building"></i></div>
            <div><div class="stat-value" id="stat-companies">0</div><div class="stat-label">{{ __('messages.companies') }}</div></div>
        </div>
        <div class="stat-card info"><div class="stat-card-icon"><i class="fas fa-file-alt"></i></div>
            <div><div class="stat-value" id="stat-applications">0</div><div class="stat-label">{{ __('messages.applications') }}</div></div>
        </div>
    </div>

    <div class="card">
        <div class="card-header">
            <span class="card-title">🚀 {{ __('messages.recent_jobs_live') }}</span>
            <span style="font-size:.75rem;color:var(--text-muted)" id="last-update">—</span>
        </div>
        <div class="card-body" id="recent-jobs">
            <div style="text-align:center;padding:2rem;color:var(--text-muted)">{{ __('messages.loading') }}</div>
        </div>
    </div>
</div>
...
<script>
    // ── نصوص مترجمة تُمرَّر من Blade إلى الجافاسكربت ──
    const i18n = {
        connectedToServer: @json(__('messages.connected_to_server')),
        disconnected:      @json(__('messages.disconnected')),
        lastUpdate:        @json(__('messages.last_update')),
    };
    ...
    socket.on('connect', () => {
        status.innerHTML = '🟢 ' + i18n.connectedToServer;
        status.style.background = 'rgba(255,255,255,.3)';
    });

    socket.on('disconnect', () => {
        status.innerHTML = '🔴 ' + i18n.disconnected;
    });
    ...
    socket.on('stats', (data) => {
        ...
        document.getElementById('last-update').textContent =
            i18n.lastUpdate + ': ' + new Date(data.timestamp).toLocaleTimeString();
