...
Route::middleware(['auth'])->group(function () {
    ...
    Route::middleware('role:company')->prefix('company')->name('company.')->group(function () {
        ...
        // Reviews - Use ONLY ONE definition (remove the duplicate)
        Route::prefix('reviews')->name('reviews.')->group(function () {
            Route::get('/', [ReviewController::class, 'index'])->name('index');      // URL: /company/reviews, Name: company.reviews.index
            Route::get('/{id}', [ReviewController::class, 'show'])->name('show');
            Route::patch('/{id}/status', [ReviewController::class, 'updateStatus'])->name('status');
...
// ════════════════════════════════════════════════════════════════
// أضف هذه المسارات إلى routes/web.php
// ════════════════════════════════════════════════════════════════


//use App\Http\Controllers\Company\ApplicationController as CompanyApplicationController;

// ── داخل middleware(['auth']) ──────────────────────────────────

// مسارات الوظائف المقترحة (للباحث عن عمل)
Route::middleware('role:user')->prefix('user')->group(function () {
    Route::get('/recommended', [JobController::class, 'recommended'])->name('jobs.recommended');
    Route::get('/recommended/api', [JobController::class, 'recommendedApi'])->name('jobs.recommended.api');
});
